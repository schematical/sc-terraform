...
resource "aws_acm_certificate" "ctothinktank_com_cert" {
  ...
  subject_alternative_names = ["*.${aws_route53_zone.ctothinktank_com.name}"]
...
resource "aws_route53_record" "example" {
  for_each = {
    for dvo in aws_acm_certificate.ctothinktank_com_cert.domain_validation_options : dvo.domain_name => {

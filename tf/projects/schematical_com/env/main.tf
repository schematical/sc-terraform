...
module "buildpipeline" {
  ...
  private_subnet_mappings = var.private_subnet_mappings
  source_buildspec_path = "www/buildspec.yml"
  ecs_deploy_cluster_name = 'prod-v1' # var.ecs_cluster_name
...
resource "aws_iam_role_policy_attachment" "code_pipeline_iam_policy_attach" {
  ...
  policy_arn = "arn:aws:iam::aws:policy/AWSCodeDeployRoleForECS" // aws_iam_policy.code_pipeline_iam_policy.arn
}
